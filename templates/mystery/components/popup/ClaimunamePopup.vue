<template>
  <div class="modal fade" id="claimunameModal" tabindex="-1" role="dialog" aria-labelledby="claimunameModalTitle" style="display: none;" aria-hidden="true">
...
<script>
    ...
    data() {
      return {
		...
		mounted() {
			$(document).on('click', '[data-popup="claimunameModal"]', () => {
				$('#claimunameModal').addClass('open').addClass('animate');
				$(document).on('click', '#claimunameModal', (e) => {
						e.stopPropagation();
				});
		...
		beforeDestroy() {
			$(document).off('click', '[data-popup="claimunameModal"]');
		},
		computed: {
      userData() {
